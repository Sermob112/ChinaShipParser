...
def safe_get(driver, url: str, wait_selector: tuple[By, str], timeout: int = 90) -> None:
    """Открыть URL и дождаться ключевого элемента. Переживает медленный рендерер."""
    # c запасом таймауты — реальный рендер может тупить
    driver.set_page_load_timeout(timeout)
    driver.set_script_timeout(timeout)
    try:
        driver.get(url)
    except TimeoutException:
        # Иногда Chrome «подвисает» на ресурсе, но страница уже почти готова.
        # Продолжаем — ниже будет явное ожидание по элементу формы.
        pass
    # Ждем, пока появится нужный элемент (например, селект роли)
...
class ChinaShipbuildingRegistrator:
    def __init__(
        ...
        last = self._read_last_index()
        self._index_counter = itertools.count(last + 1)
        # --- Фабрика драйверов ---
            ...
            if not os.path.exists(p):
                with open(p, "w", encoding="utf-8") as f:
                    if p.endswith(".jsonl"):
                        ...
                        f.write("")
    def _read_last_index(self) -> int:
        """Читает jsonl и возвращает максимальный index (или количество строк, если поля index нет)."""
        path = self.output.jsonl_path
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    obj = json.loads(line)
                    if isinstance(obj, dict) and "index" in obj and isinstance(obj["index"], int):
                        if obj["index"] > max_idx:
                            max_idx = obj["index"]
                except Exception:
                    # битая строка — игнорируем
                    pass
        # если индексов в файле нет, используем количество строк
        return max_idx if max_idx > 0 else lines

    def _next_index(self) -> int:
        # потокобезопасно вытаскиваем следующий индекс из счетчика
        with self.lock:
            return next(self._index_counter)
    ...
    def _check_success_registration(self, driver) -> bool:
        """Ждем до 60с появления #content_lbAct и валидируем текст (регистрация/опечатка + Logon)."""
        try:
            el = WebDriverWait(driver, 60).until(
                EC.presence_of_element_located((By.ID, "content_lbAct"))
            )
            t = (el.text or "").strip().lower()
            ok_phrase = ("thanks for registration" in t) or ("thanks for registeration" in t)
            has_logon = ("logon" in t) or ("login" in t) or ("log on" in t)
            return ok_phrase and has_logon
        except TimeoutException:
            return False
    ...
    def register_single_account(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # НАВИГАЦИЯ: ждем именно селект роли (страница тяжелая)
            safe_get(driver, url, (By.ID, "content_ctl_register_lst_role"), timeout=120)

            for attempt in range(1, self.max_retries + 1):
                try:
                    _rand_sleep(0.6, 1.4)  # легкая рандомизация
                    self._submit_form(driver, data)

                    # Минимум 10с под капот: сайт тормозит → даем время на сервер/рендер
                    time.sleep(10)

                    # А теперь — до 60с ждем целевой баннер успеха
                    ...
                    raise RuntimeError("Сайт не показал сообщение об успешной регистрации")

                except Exception as e:
                    if attempt < self.max_retries:
                        delay = 2.0 * attempt + random.uniform(0.5, 1.5)
...
if __name__ == "__main__":
    registrator = ChinaShipbuildingRegistrator(
        ...
        headless=False,
        output=OutputConfig(
            jsonl_path="Registrator/shipbuilding_accounts.jsonl",
            snapshot_json_path="Registrator/shipbuilding_accounts.json",
            txt_path="Registrator/shipbuilding_accounts.txt",
    ...
    try:
        registrator.register_accounts_multithreaded(count=250, max_workers=6)
        print("\n" + "=" * 50)
